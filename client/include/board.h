...
public:
  explicit Board(std::shared_ptr<client::Client> client_ = nullptr,
...
private:
  enum class status { laying_out_cards, spells_applying, dead_player };
  std::shared_ptr<client::Client> client;
  Ui::Board *ui;
  status game_status;
  std::map<status, QString> game_status_info = {
      {status::laying_out_cards,
       "Select 1 - 3 cards of different types for the move and press DO MOVE"},
      {status::spells_applying,
       "There is a showdown of cards and the use of spells of wizards"},
      {status::dead_player,
       "You're dead. Now you can only watch the battle of the living wizards"}};
  ;
  ...
  const QString not_clicked_style =
      ...
      "solid #927862;";
  const QString clicked_style = "border-radius: 20px;background-color: "
                                "rgb(252, 244, 206);border: 4px solid #B22222;";
};
