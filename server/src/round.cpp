...
bool compare_two_players(std::shared_ptr<player::Player> a,
                         std::shared_ptr<player::Player> b) {
  ...
  for (auto i : a->get_spell()) {
    priority_of_the_turn_a =
        std::max(i->get_priority_of_the_turn(), priority_of_the_turn_a);
    priority_of_the_turn_b =
        std::max(i->get_priority_of_the_turn(), priority_of_the_turn_b);
  }
...
int Round::count_of_alive_players() { return alive_players.size(); }
...
int Round::count_the_number_of_dices(
    std::vector<std::shared_ptr<card::Card>> &cur_spell,
    std::shared_ptr<card::Card> &cur_card) {
  int result = 0;
  for (auto i : cur_spell) {
    if (i->get_type_of_the_spell_component() ==
        cur_card->get_type_of_the_spell_component()) {
      result++;
    }
  }
  return result;
...
void Round::play_cards(std::shared_ptr<player::Player> gamer) {
  for (auto i : gamer->get_spell()) {
    ...
    send_logs_func(QString::fromStdString(gamer->get_name()), to, dice_result,
                   QString::fromStdString(i->convert_type_in_string(
                       i->get_type_of_the_spell_component())),
                   i->get_number());
  }
...
std::shared_ptr<player::Player> Round::get_the_strongest_player(
    std::shared_ptr<player::Player> &current_player) {
  int life = 0;
  std::shared_ptr<player::Player> strongest_player = current_player;
  for (auto &player : alive_players) {
    if (player.get()->get_lives() > life &&
        current_player.get() != player.get()) {
...
Round::get_the_weakest_player(std::shared_ptr<player::Player> &current_player) {
  int life = 200;
  std::shared_ptr<player::Player> weakest_player = current_player;
  for (auto &player : alive_players) {
    if (player.get()->get_lives() <
        life /*&& player.get() != current_player.get()*/) {
...
QList<QString> Round::damage_to_the_strongest_player(
    std::shared_ptr<player::Player> &current_player, int sum, int type) {
  QList<QString> players_with_damage;
  std::shared_ptr<player::Player> strongest_player =
      get_the_strongest_player(current_player);
  int num = get_num_of_player_in_circle(strongest_player);

  std::shared_ptr<player::Player> right_neighbour =
      alive_players[(num + 1) % count_of_alive_players()];
  std::shared_ptr<player::Player> left_neighbour =
      alive_players[(count_of_alive_players() + num - 1) %
                    count_of_alive_players()];
    ...
    if (sum >= 1 && sum <= 4) {
      ...
      if (sum >= 10) {
        ...
        if (delivery_num != -1 && num > current_player_num) {
          std::shared_ptr<card::Card> delivery_card =
              strongest_player.get()->get_spell()[delivery_num];
...
QList<QString> Round::damage_to_the_weakest_player(
    std::shared_ptr<player::Player> &current_player, int sum,
    [[maybe_unused]] int type) {
  QList<QString> players_with_damage;
  // card Mercy Killing (Delivery_2.png)
  std::shared_ptr<player::Player> weakest_player =
      get_the_weakest_player(current_player);
  if (sum >= 1 && sum <= 4) {
  ...
  players_with_damage.append(
      QString::fromStdString(weakest_player.get()->get_name()));
  return players_with_damage;
}

QList<QString> Round::damage_to_the_left_neighbour(
    std::shared_ptr<player::Player> &current_player, int sum,
    [[maybe_unused]] int type) {
  QList<QString> players_with_damage;
  // card Fist O'Nature (Delivery_7.png)

  int num = get_num_of_player_in_circle(
      current_player); // num of current player in circle
  ...
  players_with_damage.append(
      QString::fromStdString(left_neighbour.get()->get_name()));
  return players_with_damage;
}

QList<QString> Round::damage_to_the_right_neighbour(
    std::shared_ptr<player::Player> &current_player, int sum, int type) {
  QList<QString> players_with_damage;
  ...
  players_with_damage.append(
      QString::fromStdString(right_neighbour.get()->get_name()));
  return players_with_damage;
}

QList<QString>
Round::damage_to_random_foe(std::shared_ptr<player::Player> &current_player,
  ...
  for (auto &player : alive_players) {
    if (player.get()->get_name() !=
        current_player.get()->get_name()) { // all players have different names
      if (type == 2) {
  ...
  // type = 1 - card Professor Presto's (Source_2.png) and card Mind-Altering
  // (Quality_4.png)
  if (type == 1) {
  ...
  players_with_damage.append(
      QString::fromStdString(random_foe.get()->get_name()));
  return players_with_damage;
}

QList<QString>
Round::hp_to_current_player(std::shared_ptr<player::Player> &current_player,
                            int sum, int type) {
  QList<QString> players_with_damage;
...
QList<QString>
Round::damage_for_several_foes(std::shared_ptr<player::Player> &current_player,
                               int sum, int type) {
  QList<QString> players_with_damage;
  ...
  if (type == 1) {
    for (auto &player : alive_players) {
      ...
      if (foe_lives > cur_lives) {
        ...
        players_with_damage.append(
            QString::fromStdString(player.get()->get_name()));
      }
    }
  }
  ...
  if (type == 2) {
    std::shared_ptr<player::Player> left_neighbour =
        alive_players[(count_of_alive_players() + num - 1) %
                      count_of_alive_players()];
    std::shared_ptr<player::Player> left_left_neighbour =
        alive_players[(count_of_alive_players() + num - 2) %
                      count_of_alive_players()];
    players_with_damage.append(
        QString::fromStdString(left_neighbour.get()->get_name()));
    if (count_of_alive_players() > 2) {
      players_with_damage.append(
          QString::fromStdString(left_left_neighbour.get()->get_name()));
    }
    if (sum >= 1 && sum <= 4) {
      left_neighbour.get()->subtract_lives(1);
      if (count_of_alive_players() > 2) {
  ...
  if (type == 3) {
    ...
    for (auto &player : alive_players) {
      if (player.get()->get_name() !=
          current_player.get()
              ->get_name()) { // all players have different names
        players_with_damage.append(
            QString::fromStdString(player.get()->get_name()));
        if (sum >= 5 && sum <= 9) {
  ...
  // type = 4 - card Snakedance (Delivery_12.png), type = 6 - card Cone of Acid
  // (Delivery_11.png)
  if (type == 4 || type == 6) {
    players_with_damage.append(
        QString::fromStdString(right_neighbour.get()->get_name()));
  ...
  if (type == 5) {
    ...
    if (delivery_num != -1) {
      std::shared_ptr<card::Card> delivery_card =
          current_player.get()->get_spell()[delivery_num];
      ...
      for (auto &player : alive_players) {
        ...
        if (player.get()->get_name() != current_player.get()->get_name()) {
          ...
          if (delivery_card.get()->get_number() == 4 ||
              delivery_card.get()->get_number() == 7) {
            if (sum >= 1 && sum <= 4) {
              player.get()->subtract_lives(1);
            }
            if (sum >= 5 && sum <= 9) {
      ...
      if (delivery_card.get()->get_number() == 11 && (sum >= 5 && sum <= 9)) {
        ...
        std::mt19937 rng(dev());
        std::uniform_int_distribution<std::mt19937::result_type> roll(
            0, count_of_cards - 1);
        int num_of_card = roll(rng);
        current_player.get()->add_card_to_spell(
            current_player.get()->get_cards()[num_of_card]);
      }
...
QList<QString>
Round::type_of_cards_damage(std::shared_ptr<player::Player> &current_player,
                            [[maybe_unused]] int sum, int type) {
  QList<QString> players_with_damage;
  ...
  if (type == 2) {
    for (auto &player : alive_players) {
      int foe_lives = player.get()->get_lives();
      if (player.get()->get_name() != current_player.get()->get_name() &&
          foe_lives % 2 != 0) { // all players have different names
        players_with_damage.append(
            QString::fromStdString(player.get()->get_name()));
  ...
  if (type == 5) {
    ...
    for (int i = 0; i < unique; i++) {
      ...
      players_with_damage.append(
          QString::fromStdString(random_player.get()->get_name()));
    }
  }
...
QList<QString>
Round::damage_without_parametrs(std::shared_ptr<player::Player> &current_player,
                                [[maybe_unused]] int sum, int type) {
  QList<QString> players_with_damage;
  std::shared_ptr<player::Player> strongest_player =
      get_the_strongest_player(current_player);
  ...
  if (type == 5) {
    int num = get_num_of_player_in_circle(current_player);
    for (int i = 0; i < num; i++) {
      if (alive_players[i].get()->get_name() !=
          current_player.get()->get_name()) {
        alive_players[i].get()->subtract_lives(3);
        players_with_damage.append(
            QString::fromStdString(alive_players[i].get()->get_name()));
      }
    }
  }
  ...
  if (type == 6) {
    ...
    while (players_without_damage > 0) {
      std::shared_ptr<player::Player> left_neighbour =
          alive_players[(count_of_alive_players() + num - i) %
                        count_of_alive_players()];
...
QList<QString>
Round::copy_the_text_of_card(std::shared_ptr<player::Player> &current_player,
                             int sum, int type) {
  QList<QString> players_with_damage;
  ...
  if (type == 1) {
    ...
    if (kind == 0) {
      ...
      if (num_of_source != -1) {
        std::shared_ptr<card::Card> executable_card =
            current_player.get()->get_cards()[num_of_source];
    ...
    // kind = 1 - copy text of Delivery card
      ...
      if (num_of_delivery != -1) {
        std::shared_ptr<card::Card> executable_card =
            current_player.get()->get_cards()[num_of_delivery];
        return do_card_effects(executable_card, current_player, sum);
      }
    }
  }
...
QList<QString>
Round::change_spell(std::shared_ptr<player::Player> &current_player,
                    [[maybe_unused]] int sum, int type) {
  QList<QString> players_with_damage;
  int num = get_num_of_player_in_circle(current_player);
  std::shared_ptr<player::Player> strongest_player = current_player;
  ...
  if (type == 1) {
    ...
    if (current_player.get()->get_name() !=
        strongest_player.get()->get_name()) { // we are not last in circle
  ...
  if (type == 2) {
    current_player.get()->add_lives(2);
    std::shared_ptr<player::Player> weakest_player =
        get_the_weakest_player_in_round();
    if (current_player.get()->get_name() == weakest_player.get()->get_name()) {
      ...
      current_player.get()->add_card_to_spell(
          current_player.get()->get_cards()[rand_card]);
      current_player.get()->get_cards().erase(
          current_player.get()->get_cards().begin() + rand_card);
    }
  }
  return players_with_damage;
}
...
QList<QString>
Round::change_order(std::shared_ptr<player::Player> &current_player,
                    [[maybe_unused]] int sum, [[maybe_unused]] int type) {
  // card Impatient (Quality_7.png)
  return damage_without_parametrs(current_player, 0, 7);
}
QList<QString>
Round::do_card_effects(std::shared_ptr<card::Card> &executable_card,
                       std::shared_ptr<player::Player> &current_player,
                       int sum) {
  QList<QString> players_with_damage;
  if (executable_card->get_card_component() ==
  ...
  } else if (executable_card->get_card_component() ==
